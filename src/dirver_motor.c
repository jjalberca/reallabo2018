...
void motor_init(void){
	...
	pio_configure_pin(MOT_EN_IDX, 0 |
		PIO_PERIPH_B |
		PIO_DEFAULT);
	tc_pwm_init(MOT_PWM);
	tc_pwm_duty(0);
	pmc_enable_periph_clk(MOT_I1_PCM);
	pio_set_output(MOT_I1_PORT, MOT_I1_MASK, LOW, DISABLE, ENABLE);
...
void motor_dir(int dir){
	if(dir == 0){
		pio_clear(MOT_I1_PORT, MOT_I1_MASK);
		pio_clear(MOT_I2_PORT, MOT_I2_MASK);
	} else if(dir > 0){
		pio_set(MOT_I1_PORT, MOT_I1_MASK);
		pio_clear(MOT_I2_PORT, MOT_I2_MASK);
	} else if(dir < 0){
		pio_clear(MOT_I1_PORT, MOT_I1_MASK);
