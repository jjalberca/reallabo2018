...
\begin{table}[hbt]
	\begin{center}
		\begin{tabular}{c c}
			...
			\end{tabular}
	\end{center}
	\caption{Valores de tension equivalentes calculados}
	\label{veq}
\end{table}
...
\begin{figure}[htbp]
	\begin{center}
		\includegraphics[width=8cm]{curva_polinomica}
		\caption{}
		\label{polinomioA}
	\end{center}
\end{figure}
...
Finalmente, susstituyendo los valores obtenidos anteriormente en la ecuacion \ref{trans} se obtiene:
...
del problema y se solucionó. Parece ser que la interrupcción que se había activado para el control del tiempo por milisegundos a veces
no era lo bastante rápida ya que la interrupcción encargada de tomar datos en los flancos de subida o bajada de los pines de los que obtenemos
los datos de movimiento del motor parecía ser más rápida, por lo que tomaba dos datos con el mismo tiempo, por lo que al calcular la velocidad se obtenian infinitos.
...
También tuvimos ciertos problemas con la activación de las interrupcciones generadas por \emph{timers}, ya que la nomenclatura usada en el \emph{framework} no es nada clara.
...
sin embargo, tanto el el driver del PCM como en controlador de las interrupciones (NVIC), existen 9 timers (TC0, TC1, ..., TC8) complétamente independientes. Por lo que era necesario
